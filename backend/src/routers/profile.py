...
from ..models import Stream, StreamMembership, StreamRole, User
...
async def elevate_to_stream_admin(
    ...
    if result.rowcount == 0:
        return {"status": "ok", "message": "コードが確認されましたが、更新可能なストリームメンバーシップがありません"}
...
async def elevate_to_stream_admin_for_specific_stream(
    ...
    if not stream:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="指定されたストリームが見つかりません"
        )
    ...
    if not membership:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="このストリームのメンバーではありません"
        )
    ...
    # コードの検証
    if not secrets.compare_digest(elevate_request.code, stream_admin_code):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="無効なコードです")
...
async def update_profile(
    ...
    if profile_update.name is not None:
        if not profile_update.name.strip():
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="名前は空にできません"
            )
        update_data["name"] = profile_update.name.strip()

    if profile_update.class_name is not None:
        update_data["class_name"] = (
            profile_update.class_name.strip()
            if profile_update.class_name.strip()
            else None
        )
    ...
    if profile_update.student_number is not None:
        update_data["student_number"] = (
            profile_update.student_number.strip()
            if profile_update.student_number.strip()
            else None
        )
